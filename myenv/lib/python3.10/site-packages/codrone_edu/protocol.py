...
class Information(ISerializable):
    ...
    def parse(cls, dataArray):
        ...
        #print(data)
        #print("in protocol, mode: ", data.modeUpdate , "model: ", data.modelNumber, "version: ", data.version, "year: ", data.year, "month: ", data.month, "day: ", data.day)
...
class CpuID(ISerializable):

    def __init__(self):
        self.cpu_id = bytearray()
...
# LEDMode Start
class DroneLEDMode(Enum):
    Dimming = LightModeDrone.BodyDimming
    Fade_In = LightModeDrone.BodySunrise
    Fade_Out = LightModeDrone.BodySunset
    Blink = LightModeDrone.BodyFlicker
    Double_Blink = LightModeDrone.BodyFlickerDouble
    Rainbow = LightModeDrone.BodyRainbow

class ControllerLEDMode(Enum):
    Dimming = LightModeController.BodyDimming
    Fade_In = LightModeController.BodySunrise
    Fade_Out = LightModeController.BodySunset
    Blink = LightModeController.BodyFlicker
    Double_Blink = LightModeController.BodyFlickerDouble
    Rainbow = LightModeController.BodyRainbow
# LEDMode End
...
class Count(ISerializable):
    ...
    def __init__(self):
        self.timeSystem   = 0   # system operation time (sec) u32
        self.timeFlight     = 0 # flight time (sec) u32
        self.countTakeOff   = 0 # number of drone take offs u16
        self.countLanding   = 0 # number of landings of drone u16
        self.countAccident  = 0 # counts the number of accidents u16
    ...
    def toArray(self):
        return pack('IIHHH', self.timeSystem, self.timeFlight, self.countTakeOff, self.countLanding, self.countAccident)
    ...
    def parse(cls, dataArray):
        ...
        data.timeSystem, data.timeFlight, data.countTakeOff, data.countLanding, data.countAccident = unpack('<IIHHH', dataArray)
