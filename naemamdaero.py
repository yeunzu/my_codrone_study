...
def go_to_pos(target_pos: Iterable = (0, 0), allow_RMSE: float | int = 5, speed: int = 20):
    ...
    now_x_pos, now_y_pos = drone.get_position_data()[1:3]
    RMSE = math.sqrt((now_x_pos - target_x)**2 + (now_y_pos - target_y)**2)
    while RMSE > allow_RMSE:
        pos = drone.get_pos_x(), drone.get_pos_y(), drone.get_pos_z()
        note_position(df=df, value=pos)
        if now_x_pos - target_x < 0:
            drone.set_pitch(speed)
            if now_y_pos - target_y > 0:
                drone.set_roll(speed)
            elif now_y_pos - target_y < 0:
                drone.set_roll(-speed)
        elif now_x_pos - target_x > 0:
            drone.set_pitch(-speed)
            if now_y_pos - target_y > 0:
                drone.set_roll(speed)
            elif now_y_pos - target_y < 0:
...
def turn_body(target_angle: float | int):
    ...
    while abs(now_angle - target_angle) > 0.5: # 각도가 오차범위(0.5) 넘어가면 몸체회정
        drone.set_yaw(20)
        drone.move()
        time.sleep(TIME_DELAY)
...
if __name__ == '__main__':
    ...
    try:
        ...
        go_straight(100)
        # go_to_pos(target_pos=(100, 0), allow_RMSE=25)
        landing_assist(target_pos=(100, 0), allow_RMSE=4)
